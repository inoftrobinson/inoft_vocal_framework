...
function GanttMaster_translating() {
    this.tasks = [];
    this.deletedTaskIds = [];
    this.links = [];
...
exports.GanttMaster_translating = GanttMaster_translating;
GanttMaster_translating.prototype.init = function (workSpace) {
...
GanttMaster_translating.messages = {
...
GanttMaster_translating.prototype.createTask = function (id, name, code, level, start, duration) {
    var factory = new TaskFactory();
    return factory.build(id, name, code, level, start, duration);
};
GanttMaster_translating.prototype.getOrCreateResource = function (id, name) {
...
GanttMaster_translating.prototype.createResource = function (id, name) {
    var res = new Resource(id, name);
    this.resources.push(res);
    return res;
};
//update depends strings
GanttMaster_translating.prototype.updateDependsStrings = function () {
    //remove all deps
...
GanttMaster_translating.prototype.removeLink = function (fromTask, toTask) {
...
GanttMaster_translating.prototype.__removeAllLinks = function (task, openTrans) {
    if (openTrans)
        this.beginTransaction();
    var found = false;
    for (var i = 0; i < this.links.length; i++) {
...
//------------------------------------  ADD TASK --------------------------------------------
GanttMaster_translating.prototype.addTask = function (task, row) {
 ...
 * @param project
 */
GanttMaster_translating.prototype.loadProject = function (project) {
...
GanttMaster_translating.prototype.loadTasks = function (tasks, selectedRow) {
    ...
    for (var i = 0; i < this.tasks.length; i++) {
        ...
        if (!task.setPeriod(task.start, task.end)) {
            alert(GanttMaster_translating.messages.GANNT_ERROR_LOADING_DATA_TASK_REMOVED + "\n" + task.name);
            //remove task from in-memory collection
            this.tasks.splice(task.getRow(), 1);
        }
        else {
...
GanttMaster_translating.prototype.getTask = function (taskId) {
    var ret;
    for (var i = 0; i < this.tasks.length; i++) {
...
GanttMaster_translating.prototype.getResource = function (resId) {
    var ret;
...
GanttMaster_translating.prototype.changeTaskDeps = function (task) {
    return task.moveTo(task.start, false, true);
};
GanttMaster_translating.prototype.changeTaskDates = function (task, start, end) {
    //console.debug("changeTaskDates",task, start, end)
    return task.setPeriod(start, end);
};
GanttMaster_translating.prototype.moveTask = function (task, newStart) {
    return task.moveTo(newStart, true, true);
};
GanttMaster_translating.prototype.taskIsChanged = function () {
...
GanttMaster_translating.prototype.checkButtonPermissions = function () {
...
GanttMaster_translating.prototype.redraw = function () {
    this.editor.redraw();
    this.gantt.redraw();
};
GanttMaster_translating.prototype.reset = function () {
...
GanttMaster_translating.prototype.showTaskEditor = function (taskId) {
    var task = this.getTask(taskId);
    task.rowElement.find(".edit").click();
};
GanttMaster_translating.prototype.saveProject = function () {
    return this.saveGantt(false);
};
GanttMaster_translating.prototype.saveGantt = function (forTransaction) {
...
GanttMaster_translating.prototype.markUnChangedTasksAndAssignments = function (newProject) {
    ...
    if (this.__undoStack.length > 0) {
        ...
        for (var i = 0; i < newProject.tasks.length; i++) {
            ...
            if (typeof (newTask.id) == "string" && !newTask.id.startsWith("tmp_")) {
                ...
                var taskChanged = oldTask.id != newTask.id ||
...
GanttMaster_translating.prototype.loadCollapsedTasks = function () {
    var collTasks = [];
    if (localStorage) {
...
GanttMaster_translating.prototype.storeCollapsedTasks = function () {
...
GanttMaster_translating.prototype.updateLinks = function (task) {
        ...
        for (var j = 0; j < deps.length; j++) {
            ...
            if (sup) {
                if (parents && parents.indexOf(sup) >= 0) {
                    this.setErrorOnTransaction("\"" + task.name + "\"\n" + GanttMaster_translating.messages.CANNOT_DEPENDS_ON_ANCESTORS + "\n\"" + sup.name + "\"");
                    todoOk = false;
                }
                else if (descendants && descendants.indexOf(sup) >= 0) {
                    this.setErrorOnTransaction("\"" + task.name + "\"\n" + GanttMaster_translating.messages.CANNOT_DEPENDS_ON_DESCENDANTS + "\n\"" + sup.name + "\"");
                    todoOk = false;
                }
                else if (isLoop(sup, task, visited)) {
                    todoOk = false;
                    this.setErrorOnTransaction(GanttMaster_translating.messages.CIRCULAR_REFERENCE + "\n\"" + task.id + " - " + task.name + "\" -> \"" + sup.id + " - " + sup.name + "\"");
                }
                else if (depsEqualCheck.indexOf(sup) >= 0) {
                    this.setErrorOnTransaction(GanttMaster_translating.messages.CANNOT_CREATE_SAME_LINK + "\n\"" + sup.name + "\" -> \"" + task.name + "\"");
                    todoOk = false;
                }
                else {
...
GanttMaster_translating.prototype.moveUpCurrentTask = function () {
    var self = this;
...
GanttMaster_translating.prototype.moveDownCurrentTask = function () {
    var self = this;
...
GanttMaster_translating.prototype.outdentCurrentTask = function () {
    var self = this;
    if (self.currentTask) {
...
GanttMaster_translating.prototype.indentCurrentTask = function () {
    var self = this;
    if (self.currentTask) {
...
GanttMaster_translating.prototype.addBelowCurrentTask = function () {
    var self = this;
...
GanttMaster_translating.prototype.addAboveCurrentTask = function () {
    var self = this;
...
GanttMaster_translating.prototype.deleteCurrentTask = function (taskId) {
...
GanttMaster_translating.prototype.collapseAll = function () {
...
GanttMaster_translating.prototype.fullScreen = function () {
...
GanttMaster_translating.prototype.expandAll = function () {
...
GanttMaster_translating.prototype.collapse = function (task, all) {
...
GanttMaster_translating.prototype.expand = function (task, all) {
...
GanttMaster_translating.prototype.getCollapsedDescendant = function () {
...
GanttMaster_translating.prototype.addIssue = function () {
    var self = this;
    if (self.currentTask && self.currentTask.isNew()) {
        alert(GanttMaster_translating.messages.PLEASE_SAVE_PROJECT);
        return;
    }
...
GanttMaster_translating.prototype.openExternalEditor = function () {
...
GanttMaster_translating.prototype.beginTransaction = function () {
...
GanttMaster_translating.prototype.setErrorOnTransaction = function (errorMessage, task) {
    if (this.__currentTransaction) {
...
GanttMaster_translating.prototype.isTransactionInError = function () {
    if (!this.__currentTransaction) {
...
GanttMaster_translating.prototype.endTransaction = function () {
    if (!this.__currentTransaction) {
        console.error("Transaction never started.");
        return true;
    }
...
// inhibit undo-redo
GanttMaster_translating.prototype.checkpoint = function () {
...
GanttMaster_translating.prototype.undo = function () {
...
GanttMaster_translating.prototype.redo = function () {
...
GanttMaster_translating.prototype.saveRequired = function () {
...
GanttMaster_translating.prototype.print = function () {
    this.gantt.redrawTasks(true);
    print();
};
GanttMaster_translating.prototype.resize = function () {
    var self = this;
...
GanttMaster_translating.prototype.scrolled = function (oldFirstRow) {
    var self = this;
 ...
 * @return {*}
 */
GanttMaster_translating.prototype.computeCriticalPath = function () {
    if (!this.tasks)
        return false;
...
GanttMaster_translating.prototype.manageSaveRequired = function (ev, showSave) {
 ...
 */
GanttMaster_translating.prototype.setHoursOn = function (startWorkingHour, endWorkingHour, dateFormat, resolution) {
