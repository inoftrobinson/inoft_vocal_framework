def get_function_from_file_and_path(file_filepath: str, function_path_qualname: str):
    ...
    if not isfile(file_filepath):
        ...
        module_spec = importlib.util.spec_from_file_location("file_containing_callback", file_filepath)
        if module_spec is not None:
            module_file = importlib.util.module_from_spec(module_spec)
            module_spec.loader.exec_module(module_file)

            function_path_list = function_path_qualname.split(".")
            if len(function_path_list) > 0:
                vars_module_file = vars(module_file)
                if function_path_list[0] in vars_module_file:
                    ...
                    return get_nested_class_or_function(current_nested_class_or_function=vars_module_file[function_path_list[0]],
                                                        function_path_remaining_elements=function_path_list[1:])
    return None
